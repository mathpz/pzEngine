...
namespace pz
{
    ...
    PzPipeline::~PzPipeline()
    {
        vkDestroyShaderModule(pzDevice.device(), vertexShaderModule, nullptr);
        vkDestroyShaderModule(pzDevice.device(), fragmentShaderModule, nullptr);
        vkDestroyPipeline(pzDevice.device(), graphicsPipeline, nullptr);
    }
    ...
    void PzPipeline::createGraphicsPipeline(const std::string& vertexShaderFilepath, const std::string& fragmentShaderFilepath, const PipelineConfigInfo& configInfo)
    {
        ...
        VkPipelineShaderStageCreateInfo shaderStages[2]; // 2 stages, vertex and fragment
        // Vertex
        ...
        if(vkCreateGraphicsPipelines(pzDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS)
        {
    ...
    void PzPipeline::defaultPipelineConfigInfo(PipelineConfigInfo& configInfo)
    {
        // inputs are a list of numbers broken up per vertex in this case 3 vertices with 2 properties each, but may have more properties
        //
        //      (a, b) .
        //                                =>   a, b,     c, d,    e, f,
        ...
        configInfo.inputAssemblyInfo.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;  // each 3 vertices forms a triangle
        ...
        configInfo.rasterizationInfo.frontFace = VK_FRONT_FACE_CLOCKWISE;           // which face should be considered front facing
        ...
        // Colorblending (how we combine color in the frame buffer)
    ...
    void PzPipeline::enableAlphaBlending(PipelineConfigInfo& configInfo)
    {
